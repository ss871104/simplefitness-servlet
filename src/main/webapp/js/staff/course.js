...
(() => {
  ...
  // 拿課程名稱
  fetch("/simplefitness-servlet/courseList/getAllCourse")
    .then((resp) => resp.json())
    .then((courseList) => {
  ...
  document.getElementById("queryCourse").addEventListener("click", () => {
    ...
    // 列出現有課程
    fetch("/simplefitness-servlet/course/searchCourse", {
      method: "POST",
          ...
          for (i = 0; i < course["length"]; i++) {
                      ...
                      <table class="table table-hover">
                          ...
                          <tbody id="courListBody${date}">
                            <tr id=course${course[i].courseId}>
                              ...
                              <td><button type="button" class="btn btn-secondary" id="memList${course[i].courseId}" value="${course[i].courseId}">查看已報名清單</button></td>
                            </tr>
                          </tbody>
                      </table>
                  </div>
            ...
            document.querySelector(`#edit${courseData.courseId}`).onclick = () => {
              ...
              document.querySelector("#editSave").onclick = () => {
                ...
                if(moment(editCourseDate.value).isAfter(now)) {
                  fetch("/simplefitness-servlet/course/editCourse",
                  {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
            ...
            document.querySelector(`#delete${course[i].courseId}`).onclick = () => {
              ...
              document.querySelector("#delete").onclick = () => {
                ...
                fetch("/simplefitness-servlet/course/deleteCourse", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
            ...
            // 點擊查看報名清單
            const button = document.querySelector(`#memList${course[i].courseId}`);
            button.addEventListener('click', () => {
              sessionStorage.setItem('course', button.value);
              location = './course_member_list.html';
            });
          }
  ...
  document.getElementById("add").addEventListener("click", () => {
    ...
    if(moment(newCourseDate.value).isAfter(now)) {
      ...
      fetch("/simplefitness-servlet/course/addCourse", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          gymId: newGym.value,
...
function EditTemplate(id,gymName,startTime,endTime,courseName,empName,statusList,public){
  ...
  <td><button type="button" class="btn btn-secondary" id="memList${id}" value="${id}">查看已報名清單</button></td>`
}
